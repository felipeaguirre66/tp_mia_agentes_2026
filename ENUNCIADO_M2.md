...
**Recencia.** Cualquiera sea la estrategia, hay una invariante que no pueden romper: el **mensaje de usuario más reciente** siempre debe aparecer en la siguiente llamada al LLM. Pueden descartar context antiguo respetar el presupuesto, pero nunca lo que el usuario acaba de decir.
...
Estrategia de memoria: cómo fue implementada y qué problemas encontraron. Si implementan una estrategia alternativa a sliding window, comenten por qué la eligieron y qué tradeoffs tiene.
Salida estructurada: cómo ofrecen final_resul` al LLM, cómo validan los argumentos, cómo se reparan los fallos de validación, qué pasa cuando se agotan los reintentos.
Errores en herramientas: qué errores recuperables detectan la calculadora y el lector de archivos, qué información devuelven al LLM para facilitar la corrección, y un ejemplo concreto de recuperación en cada una.
Modos de fallo que dejaron deliberadamente dentro vs. fuera del alcance.
